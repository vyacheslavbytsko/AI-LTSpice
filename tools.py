...
def finalize_netlist_b64_tool():
    def finalize_netlist(netlist_b64, v, analysis):
        ...
        if v == "SINE":
            new_netlist.append(f"V{num_of_Vs+1} INPUT_NODE {"V_MINUS_NODE" if flag_for_v_minus else "0"} SINE(1 1 100000) AC 1")
        else:
            new_netlist.append(f"V{num_of_Vs+1} INPUT_NODE {"V_MINUS_NODE" if flag_for_v_minus else "0"} 1")

        new_netlist.append(f"R{num_of_Rs + 1} OUTPUT_NODE 0 10")
